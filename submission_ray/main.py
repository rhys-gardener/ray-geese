...
LAST_STATE = [np.zeros(256, np.float32),
...
class CustomActor:
    def __init__(self, config, path):
        # self.agent = agent
        self.agent = ppo.PPOTrainer(config, env=HungryGeeseKaggle)
        # self.agent.restore(path)
    
    def get_action(self, obs, last_state, last_action):
        return self.agent.compute_action(obs, last_state, prev_action=last_action)[0]
    
    def get_state(self, obs, last_state, last_action):
        return  self.agent.compute_action(obs, last_state, prev_action=last_action)[1]



ray.init(configure_logging=True, logging_level='ERROR', log_to_driver=False, num_cpus = 1, local_mode=True)   
...
path = '/kaggle_simulations/agent/model/checkpoint-1'
#path = 'model/checkpoint-1'
ACTOR = CustomActor(config, path)
...
def agent(obs_dict, config_dict):
    ...
    #Actor = load_model('/kaggle_simulations/agent/model')
    ...
    obs = env_wrapper.get_geese_observation(player_index, observation)
    state = LAST_STATE
    #print(dir(ACTOR))
    print(f'last action: {LAST_ACTION}')
    action = ACTOR.get_action(obs, LAST_STATE, LAST_ACTION)
    state = ACTOR.get_state(obs, LAST_STATE, LAST_ACTION)
